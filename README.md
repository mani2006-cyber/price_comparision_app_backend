...
| POST | `/compare-url` | — | `{ url }` | `200` `{ success, result }` — `result.detectedMarketplace`, `result.matchesFound`, `result.results[]` (the original product plus cross-marketplace matches, each carrying `isOriginal` and a `similarityScore`), `result.similarProducts[]`, `result.aiSummary` |
